...
class Dataset_patch: 
    ...
    def __len__(self):
        #return len(self.all_images) // self.patch_size**2
        return 512*256*3 // self.patch_size**2
    ...
    def __getitem__(self, index):
        #ipdb.set_trace()
        x = torch.randint(low=0, high=self.W, size=[1],device='cpu')[0].item()
        y = torch.randint(low=0, high=self.H, size=[1],device='cpu')[0].item()
